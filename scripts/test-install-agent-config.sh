...
#
# This repo has no sibling-owned skills, so its EXTRA_SKILLS is empty and the installer's
# sibling-skill loop is a no-op. That is asserted directly, and the loop itself is still exercised:
# a patched COPY of the installer inside the throwaway worktree points EXTRA_SKILLS at a fake
# sibling repo, so the "link it in" and "never replace a real directory" branches stay covered.
...
# Pre-existing settings that must survive the jq merges. The real main checkout's
# settings.local.json carries a permissions.allow list, so the fixture has one too.
mkdir -p "$WT/.claude"
printf '%s' '{"permissions":{"allow":["Bash(mise exec *)"]}}' > "$WT/.claude/settings.local.json"
printf '%s' '{"hooks":{"PreToolUse":[{"matcher":"Write","hooks":[{"type":"command","command":"echo other"}]}]}}' \
  > "$WT/.claude/settings.json"

# NOTE: no /.mcp.json is seeded here on purpose — a fresh worktree genuinely has none, and the
# installer has to do the right thing whether or not the main checkout has one to link.
...
echo "== sibling skills (EXTRA_SKILLS is empty in this repo) =="
eq "EXTRA_SKILLS is empty" "EXTRA_SKILLS=()" \
  "$(grep -m1 '^EXTRA_SKILLS=' "$WT/scripts/install-agent-config.sh")"
# An empty array under `set -u` must not abort the installer, and must link nothing.
ok "the empty loop links no sibling skill" \
  bash -c '! grep -qE "^ +\.claude/skills/.+ -> \.\./" "$1"' _ "$INSTALL_LOG"
eq ".agents/skills holds exactly this repo's skills" \
  "$(ls "$WT/skills" | LC_ALL=C sort | tr '\n' ' ')" \
  "$(ls "$WT/.agents/skills" | LC_ALL=C sort | tr '\n' ' ')"

# The loop body itself, driven by a patched COPY of the installer and a fake sibling repo next to
# the worktree ($WT/../sibling-repo is $TMP/sibling-repo). This is what a repo with a non-empty
# EXTRA_SKILLS gets, and it proves a locally installed skill (a REAL directory) is never replaced.
mkdir -p "$TMP/sibling-repo/skills/fake-sibling" "$TMP/sibling-repo/skills/locally-installed"
printf -- '---\nname: fake-sibling\ndescription: stub\n---\n' \
  > "$TMP/sibling-repo/skills/fake-sibling/SKILL.md"
printf -- '---\nname: locally-installed\ndescription: stub\n---\n' \
  > "$TMP/sibling-repo/skills/locally-installed/SKILL.md"
mkdir -p "$WT/.claude/skills/locally-installed"
printf 'local\n' > "$WT/.claude/skills/locally-installed/MARKER"
sed 's|^EXTRA_SKILLS=()$|EXTRA_SKILLS=(../sibling-repo/skills/fake-sibling ../sibling-repo/skills/locally-installed ../nope/skills/absent)|' \
  "$WT/scripts/install-agent-config.sh" > "$WT/scripts/install-with-extra-skills.sh"
env HOME="$FAKEHOME_RO" bash "$WT/scripts/install-with-extra-skills.sh" > "$TMP/install-extra.log" 2>&1
eq "install with a non-empty EXTRA_SKILLS exits 0" 0 "$?"
ok "sibling skill .agents/skills/fake-sibling is a symlink" test -L "$WT/.agents/skills/fake-sibling"
ok "sibling skill .agents/skills/fake-sibling resolves"     test -e "$WT/.agents/skills/fake-sibling/SKILL.md"
ok "sibling skill .claude/skills/fake-sibling resolves"     test -e "$WT/.claude/skills/fake-sibling/SKILL.md"
ok "a real skill dir is not replaced"        test -f "$WT/.claude/skills/locally-installed/MARKER"
ok "a real skill dir stays a real dir"       test ! -L "$WT/.claude/skills/locally-installed"
ok "…but the other half is still linked"     test -L "$WT/.agents/skills/locally-installed"
ok "a sibling that is not checked out is skipped" grep -q 'skipped absent' "$TMP/install-extra.log"
rm -rf "$WT/.agents/skills/fake-sibling" "$WT/.claude/skills/fake-sibling" \
       "$WT/.agents/skills/locally-installed" "$WT/.claude/skills/locally-installed" \
       "$WT/scripts/install-with-extra-skills.sh"
...
eq "settings.local.json preserves pre-existing keys" "Bash(mise exec *)" \
...
if [ -f "$MAIN/.mcp.json" ]; then
  ...
  # This repo's main checkout has no /.mcp.json today, so the installer must say so and leave the
  # worktree without one, rather than creating a dangling symlink.
  ok "no /.mcp.json is invented when the main checkout has none" test ! -e "$WT/.mcp.json"
  ok "…and the installer says nothing to link"  grep -q 'nothing to link' "$INSTALL_LOG"
  ok "…and config.toml has no [mcp_servers]"    bash -c '! grep -q "^\[mcp_servers\." "$1"' _ "$WT/.codex/config.toml"
fi
