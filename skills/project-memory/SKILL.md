...
## Never store generated artifacts or dumps

`.agents/memory` is a working store for *facts*, not for the material `AGENTS.md` keeps out of the
repo: generated artifacts, screenshots, browser DOM dumps, parity JSON, Godot temporary project
files, build output. A memory that inlines one of those is the same disclosure by another route, and
it goes stale the moment the renderer changes. Say where the evidence was produced and what it
showed; do not paste it in.
