...
import type { ParticleConfig } from "../src/particles";
import {
  activeParticleCount,
  createParticleState,
...
function config(overrides: Partial<ParticleConfig>): ParticleConfig {
...
describe("simulateParticles — executed sub-step count", () => {
  ...
  const stepped = (over: Partial<ParticleConfig>, dt: number): number =>
    simulateParticles(
...
describe("flipbook frames", () => {
  function frames(over: Partial<ParticleConfig>): number[] {
    const state = createParticleState(
      config({
        amount: 16,
        lifetime: 1,
