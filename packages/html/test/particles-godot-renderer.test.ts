...
import type { GodotRendererBackend } from "@godot-scene-web/effects/particles";
import {
  createParticleState,
...
import type { GodotHtmlRenderOptions } from "../src/types";
...
describe("the godotRenderer option", () => {
  ...
  it("is structurally the same union as GodotRendererBackend", () => {
    // `types.ts` spells the union inline to keep its diff minimal; this pins the two
    // together so they cannot drift.
    const fromOptions: GodotHtmlRenderOptions["godotRenderer"] =
      "gl_compatibility" satisfies GodotRendererBackend;
    const fromCanonical: GodotRendererBackend = "mobile" satisfies NonNullable<
      GodotHtmlRenderOptions["godotRenderer"]
    >;
    expect([fromOptions, fromCanonical]).toEqual([
      "gl_compatibility",
      "mobile",
    ]);
  });
...
describe("the correction's scope — what it must NOT reach", () => {
  ...
  it("leaves colorRamp, colorInitialRamp and the LUT byte-for-byte untouched", () => {
    ...
    expect(cfg.colorLut).toEqual(colorLut);
